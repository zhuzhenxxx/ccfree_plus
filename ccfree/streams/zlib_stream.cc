...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    CCFREE_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    CCFREE_ASSERT((window_bits >= 8 && window_bits <= 15));
    CCFREE_ASSERT((memlevel >= 1 && memlevel <= 9));
...
ccfree::ByteArray::ptr ZlibStream::getByteArray() {
