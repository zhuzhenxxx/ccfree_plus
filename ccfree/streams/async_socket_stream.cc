...
void AsyncSocketStream::Ctx::doRsp() {
    ...
    if(!ccfree::Atomic::compareAndSwapBool(scheduler, scd, (Scheduler*)nullptr)) {
        return;
    }
    if(!scd || !fiber) {
        return;
...
bool AsyncSocketStream::start() {
    if(!m_iomanager) {
        m_iomanager = ccfree::IOManager::GetThis();
    }
    if(!m_worker) {
        m_worker = ccfree::IOManager::GetThis();
    }
...
void AsyncSocketStream::doRead() {
    ...
    CCFREE_LOG_DEBUG(g_logger) << "doRead out " << this;
    innerClose();
    m_waitSem.notify();
...
void AsyncSocketStream::doWrite() {
    ...
    CCFREE_LOG_DEBUG(g_logger) << "doWrite out " << this;
    {
        RWMutexType::WriteLock lock(m_queueMutex);
        m_queue.clear();
    }
...
bool AsyncSocketStream::innerClose() {
    CCFREE_ASSERT(m_iomanager == ccfree::IOManager::GetThis());
...
AsyncSocketStream::ptr AsyncSocketStreamManager::get() {
    ...
    for(uint32_t i = 0; i < m_size; ++i) {
        auto idx = ccfree::Atomic::addFetch(m_idx, 1);
