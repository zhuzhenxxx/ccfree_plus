...
static ccfree::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
    ccfree::Config::Lookup("tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2),
            "tcp server read timeout");

static ccfree::Logger::ptr g_logger = CCFREE_LOG_NAME("system");

TcpServer::TcpServer(ccfree::IOManager* worker,
...
bool TcpServer::bind(ccfree::Address::ptr addr, bool ssl) {
...
bool TcpServer::bind(const std::vector<Address::ptr>& addrs
    ...
    for(auto& addr : addrs) {
        ...
        if(!sock->bind(addr)) {
            CCFREE_LOG_ERROR(g_logger) << "bind fail errno="
        ...
        if(!sock->listen()) {
            CCFREE_LOG_ERROR(g_logger) << "listen fail errno="
                << errno << " errstr=" << strerror(errno)
                << " addr=[" << addr->toString() << "]";
            fails.push_back(addr);
            continue;
    ...
    for(auto& i : m_socks) {
        CCFREE_LOG_INFO(g_logger) << "type=" << m_type
            << " name=" << m_name
            << " ssl=" << m_ssl
...
void TcpServer::startAccept(Socket::ptr sock) {
    while(!m_isStop) {
        ...
        if(client) {
            ...
            CCFREE_LOG_ERROR(g_logger) << "accept errno=" << errno
