...
bool NameServerModule::handleRockRequest(ccfree::RockRequest::ptr request
                                         ,ccfree::RockResponse::ptr response
                                         ,ccfree::RockStream::ptr stream) {
    ccfree::Atomic::addFetch(s_request_count, 1);
    switch(request->getCmd()) {
        ...
        default:
            CCFREE_LOG_WARN(g_logger) << "invalid cmd=0x" << std::hex << request->getCmd();
            break;
    }
    return true;
}

bool NameServerModule::handleRockNotify(ccfree::RockNotify::ptr notify
                                        ,ccfree::RockStream::ptr stream) {
    return true;
}

bool NameServerModule::onConnect(ccfree::Stream::ptr stream) {
    ccfree::Atomic::addFetch(s_on_connect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if(!rockstream) {
        CCFREE_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    }
    auto addr = rockstream->getRemoteAddress();
    if(addr) {
        CCFREE_LOG_INFO(g_logger) << "onConnect: " << *addr;
    }
    return true;
}

bool NameServerModule::onDisconnect(ccfree::Stream::ptr stream) {
    ccfree::Atomic::addFetch(s_on_disconnect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if(!rockstream) {
        CCFREE_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    ...
    auto addr = rockstream->getRemoteAddress();
    if(addr) {
        CCFREE_LOG_INFO(g_logger) << "onDisconnect: " << *addr;
    }
...
NSClientInfo::ptr NameServerModule::get(ccfree::RockStream::ptr rs) {
    ccfree::RWMutex::ReadLock lock(m_mutex);
    auto it = m_sessions.find(rs);
    return it == m_sessions.end() ? nullptr : it->second;
}

bool NameServerModule::handleRegister(ccfree::RockRequest::ptr request
                                      ,ccfree::RockResponse::ptr response
                                      ,ccfree::RockStream::ptr stream) {
    auto rr = request->getAsPB<RegisterRequest>();
    if(!rr) {
        CCFREE_LOG_ERROR(g_logger) << "invalid register request from: "
...
void NameServerModule::set(ccfree::RockStream::ptr rs, NSClientInfo::ptr new_value) {
    if(!rs->isConnected()) {
        new_value = nullptr;
    }
    ...
    //ccfree::WorkerMgr::GetInstance()->schedule("notify",
...
std::set<ccfree::RockStream::ptr> NameServerModule::getStreams(const std::string& domain) {
    ccfree::RWMutex::ReadLock lock(m_mutex);
    auto it = m_domainToSessions.find(domain);
    return it == m_domainToSessions.end() ? std::set<ccfree::RockStream::ptr>() : it->second;
}
...
bool NameServerModule::handleQuery(ccfree::RockRequest::ptr request
                                   ,ccfree::RockResponse::ptr response
                                   ,ccfree::RockStream::ptr stream) {
    auto qreq = request->getAsPB<QueryRequest>();
    if(!qreq) {
        CCFREE_LOG_ERROR(g_logger) << "invalid query request from: "
            << stream->getRemoteAddressString();
        return false;
    }
...
void NameServerModule::setQueryDomain(ccfree::RockStream::ptr rs, const std::set<std::string>& ds) {
...
bool NameServerModule::handleTick(ccfree::RockRequest::ptr request
                                  ,ccfree::RockResponse::ptr response
                                  ,ccfree::RockStream::ptr stream) {
    return true;
}
