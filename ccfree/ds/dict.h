#ifndef __CCFREE_DS_DICT_H__
#define __CCFREE_DS_DICT_H__

#include "ccfree/ds/util.h"
#include "ccfree/util.h"
#include "ccfree/mutex.h"
...
template<class K
        ,class V
        ,class PosHash = ccfree::ds::Murmur3Hash<K>
        >
class Dict {
...
public:
    ...
    SharedArray<V> get(const K& k, bool duplicate = true) {
        ...
        ccfree::RWMutex::ReadLock lock2(s_mutex[pos % MAX_MUTEX]);
        auto it = BinarySearch(m_datas[pos].begin(), m_datas[pos].end(), Node(k));
        //CCFREE_ASSERT(it == std::find(m_datas[pos].begin(), m_datas[pos].end(), Node(k)));
    ...
    bool insert(const K& k, const V* v, const uint32_t& size) {
        ...
        ccfree::RWMutex::ReadLock lock(m_mutex);
        uint32_t pos = hashvalue % m_size;
        ccfree::RWMutex::WriteLock lock2(s_mutex[pos % MAX_MUTEX]);
        ...
        if(it == m_datas[pos].end()) {
            ...
            ccfree::Atomic::addFetch(m_total);
        } else {
            if(it->size == (int)size) {
    ...
    void foreach(callback cb) {
        ...
        for(size_t i = 0; i < m_size; ++i) {
            ccfree::RWMutex::ReadLock lock2(s_mutex[i % MAX_MUTEX]);
    ...
    bool del(const K& k) {
        ...
        if(it == m_datas[pos].end()) {
            ...
            std::sort(m_datas[pos].begin(), m_datas[pos].end());
            ccfree::Atomic::subFetch(m_total);
        }
        return true;
    }
    ...
    bool writeTo(std::ostream& os, uint64_t speed = -1) {
        ...
        if(speed == (uint64_t)-1) {
            ...
            ccfree::WriteFixToStreamWithSpeed(os, (const char*)&vs[0], size, speed);
        }
        ...
        if(speed == (uint64_t)-1) {
            ...
            ccfree::WriteFixToStreamWithSpeed(os, (const char*)&ns[0], size, speed);
        }
...
private:
    ...
    static ccfree::RWMutex s_mutex[MAX_MUTEX];
};
...
template<class K
        ...
        ,class PosHash
        >
ccfree::RWMutex Dict<K, V, PosHash>::s_mutex[MAX_MUTEX];
...
public:
    static uint64_t GetID(const std::string& str) {
        ...
        return ccfree::murmur3_hash64(str.c_str(), str.size(), 1060627423, 1050126127);
    }
    static uint64_t GetID(const char* str, const uint32_t& size) {
        if(size == 0) {
            return 0;
        }
        return ccfree::murmur3_hash64(str, size, 1060627423, 1050126127);
    }
    static uint64_t GetID(const char* str) {
        if(str == nullptr) {
            return 0;
        }
        return ccfree::murmur3_hash64(str, 1060627423, 1050126127);
    }
