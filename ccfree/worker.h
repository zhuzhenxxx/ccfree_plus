#ifndef __CCFREE_WORKER_H__
#define __CCFREE_WORKER_H__
...
public:
    ...
    static WorkerGroup::ptr Create(uint32_t batch_size, ccfree::Scheduler* s = ccfree::Scheduler::GetThis()) {
        return std::make_shared<WorkerGroup>(batch_size, s);
    }

    WorkerGroup(uint32_t batch_size, ccfree::Scheduler* s = ccfree::Scheduler::GetThis());
    ~WorkerGroup();
...
public:
    ...
    void schedule(const std::string& name, FiberOrCb fc, int thread = -1) {
        ...
        if(s) {
            ...
            static ccfree::Logger::ptr s_logger = CCFREE_LOG_NAME("system");
            CCFREE_LOG_ERROR(s_logger) << "schedule name=" << name
                << " not exists";
        }
    }
...
typedef ccfree::Singleton<WorkerManager> WorkerMgr;
