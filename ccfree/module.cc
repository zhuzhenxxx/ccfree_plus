...
static ccfree::ConfigVar<std::string>::ptr g_module_path
...
bool Module::handleRequest(ccfree::Message::ptr req
                           ,ccfree::Message::ptr rsp
                           ,ccfree::Stream::ptr stream) {
    CCFREE_LOG_DEBUG(g_logger) << "handleRequest req=" << req->toString()
            << " rsp=" << rsp->toString() << " stream=" << stream;
    return true;
}

bool Module::handleNotify(ccfree::Message::ptr notify
                          ,ccfree::Stream::ptr stream) {
    CCFREE_LOG_DEBUG(g_logger) << "handleNotify nty=" << notify->toString()
            << " stream=" << stream;
    return true;
}
...
bool Module::onConnect(ccfree::Stream::ptr stream) {
    return true;
}

bool Module::onDisconnect(ccfree::Stream::ptr stream) {
    return true;
}
...
bool RockModule::handleRequest(ccfree::Message::ptr req
                               ,ccfree::Message::ptr rsp
                               ,ccfree::Stream::ptr stream) {
    auto rock_req = std::dynamic_pointer_cast<ccfree::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<ccfree::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<ccfree::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(ccfree::Message::ptr notify
                              ,ccfree::Stream::ptr stream) {
    auto rock_nty = std::dynamic_pointer_cast<ccfree::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<ccfree::RockStream>(stream);
...
void ModuleManager::init() {
    ...
    ccfree::FSUtil::ListAllFile(files, path, ".so");
