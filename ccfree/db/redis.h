#ifndef __CCFREE_DB_REDIS_H__
#define __CCFREE_DB_REDIS_H__
...
public:
    ...
    FoxRedis(ccfree::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedis();
...
private:
    struct FCtx {
        std::string cmd;
        ccfree::Scheduler* scheduler;
        ccfree::Fiber::ptr fiber;
        ReplyPtr rpy;
    };
...
public:
    ...
    FoxRedisCluster(ccfree::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedisCluster();
...
typedef ccfree::Singleton<RedisManager> RedisMgr;
