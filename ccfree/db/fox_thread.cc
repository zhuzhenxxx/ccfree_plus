...
static ccfree::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_thread_info_set
...
void FoxThread::read_cb(evutil_socket_t sock, short which, void* args) {
    ...
    if(recv(sock, cmd, sizeof(cmd), 0) > 0) {
        ...
        for(auto it = callbacks.begin();
                it != callbacks.end(); ++it) {
            if(*it) {
                //CCFREE_ASSERT(thread == GetThis());
                try {
                    (*it)();
                } catch (std::exception& ex) {
                    CCFREE_LOG_ERROR(g_logger) << "exception:" << ex.what();
                } catch (const char* c) {
                    CCFREE_LOG_ERROR(g_logger) << "exception:" << c;
                } catch (...) {
                    CCFREE_LOG_ERROR(g_logger) << "uncatch exception";
                }
            } else {
        ...
        ccfree::Atomic::addFetch(thread->m_total, callbacks.size());
...
FoxThread::FoxThread(const std::string& name, struct event_base* base)
    ...
    if(evutil_socketpair(AF_UNIX, SOCK_STREAM, 0, fds) == -1) {
        //CCFREE_LOG_ERROR(g_logger) << "FoxThread init error";
...
void FoxThread::start() {
    if(m_thread) {
        //CCFREE_LOG_ERROR(g_logger) << "FoxThread is running";
...
bool FoxThreadPool::dispatch(callback cb) {
    do {
        ccfree::Atomic::addFetch(m_total, (uint64_t)1);
        RWMutex::WriteLock lock(m_mutex);
        if(!m_advance) {
...
bool FoxThreadPool::batchDispatch(const std::vector<callback>& cbs) {
    ccfree::Atomic::addFetch(m_total, cbs.size());
    RWMutex::WriteLock lock(m_mutex);
    if(!m_advance) {
        for(auto cb : cbs) {
...
const std::string& FoxThread::GetFoxThreadName() {
    ...
    uint64_t tid = ccfree::GetThreadId();
    do {
...
void FoxThread::setThis() {
    m_name = m_name + "_" + std::to_string(ccfree::GetThreadId());
    s_thread = this;

    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names[ccfree::GetThreadId()] = m_name;
}

void FoxThread::unsetThis() {
    s_thread = nullptr;
    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names.erase(ccfree::GetThreadId());
}
...
void FoxThreadManager::init() {
    ...
    for(auto i : m) {
        auto num = ccfree::GetParamValue(i.second, "num", 0);
        auto name = i.first;
        auto advance = ccfree::GetParamValue(i.second, "advance", 0);
        if(num <= 0) {
            CCFREE_LOG_ERROR(g_logger) << "thread pool:" << name
                        << " num:" << num
                        << " advance:" << advance
                        << " invalid";
            continue;
        ...
        if(num == 1) {
            ...
            CCFREE_LOG_INFO(g_logger) << "init thread : " << i.first;
        } else {
            m_threads[i.first] = FoxThreadPool::ptr(new FoxThreadPool(
                            num, name, advance));
            CCFREE_LOG_INFO(g_logger) << "init thread pool:" << name
                       << " num:" << num
                       << " advance:" << advance;
        }
    }
...
void FoxThreadManager::start() {
    for(auto i : m_threads) {
        CCFREE_LOG_INFO(g_logger) << "thread: " << i.first << " start begin";
        i.second->start();
        CCFREE_LOG_INFO(g_logger) << "thread: " << i.first << " start end";
    }
}

void FoxThreadManager::stop() {
    for(auto i : m_threads) {
        CCFREE_LOG_INFO(g_logger) << "thread: " << i.first << " stop begin";
        i.second->stop();
        CCFREE_LOG_INFO(g_logger) << "thread: " << i.first << " stop end";
    }
    for(auto i : m_threads) {
        CCFREE_LOG_INFO(g_logger) << "thread: " << i.first << " join begin";
        i.second->join();
        CCFREE_LOG_INFO(g_logger) << "thread: " << i.first << " join end";
    }
}
