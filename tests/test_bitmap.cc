...
#include "ccfree/ds/bitmap.h"
#include "ccfree/ds/roaring_bitmap.h"
...
void init(uint32_t size, std::set<uint32_t> v = {}) {
    ...
    std::set<uint32_t> v0;
    ccfree::ds::Bitmap::ptr b;
    if(!size) {
        size = rand() % 4096;
        b.reset(new ccfree::ds::Bitmap(size));
    ...
    ccfree::ds::Bitmap::ptr c(new ccfree::ds::Bitmap(size, 0xFF));
    *c &= *b;
    CCFREE_ASSERT(c->getCount() == v0.size());
    ...
    //};
    //uint32_t size = 328;
    //ccfree::ds::Bitmap::ptr b(new ccfree::ds::Bitmap(size));
    //for(auto& i : vv) {
    //    b->set(i, true);
    //}
    ...
    ccfree::ds::Bitmap::ptr bb(new ccfree::ds::Bitmap(size, 0xFF));
    *b &= *bb;
#define XX(v) \
        CCFREE_LOG_INFO(g_logger) << #v ": " << ccfree::Join(v.begin(), v.end(), ",");
...
#define XX_ASSERT(a, b) \
    ...
    if(v2 != v4 || v1 != v3) {
        ...
        XX_ASSERT(v00, v3);
        XX_ASSERT(v000, v4);
        //CCFREE_ASSERT(v00 == v3);
        //CCFREE_ASSERT(v1 == v3);
        //CCFREE_ASSERT(v2 == v4);
#undef XX_ASSERT
#undef XX
    }
    //CCFREE_LOG_INFO(g_logger) << "size: " << size;
}
...
void test_compress(size_t size, std::set<uint32_t> v = {}) {
    ...
    if(*b != *bb) {
        CCFREE_LOG_INFO(g_logger) << "b  :" << b->toString();
        CCFREE_LOG_INFO(g_logger) << "bc :" << bc->toString();
        CCFREE_LOG_INFO(g_logger) << "bb :" << bb->toString();
        CCFREE_LOG_INFO(g_logger) << "size=" << size << " - " << ccfree::Join(v.begin(), v.end(), ",");
...
void test_op(size_t size, std::set<uint32_t> v1, std::set<uint32_t> v2) {
    ccfree::ds::Bitmap::ptr a;
    ccfree::ds::Bitmap::ptr b;

    if(size == 0) {
        size = rand() % 4096;
        a.reset(new ccfree::ds::Bitmap(size));
        b.reset(new ccfree::ds::Bitmap(size));
    ...
    CCFREE_ASSERT(a->cross(*b) == a->cross(*bc));
    ...
    CCFREE_ASSERT(a->cross(*b) == (!and_sv.empty()));
...
int main(int argc, char** argv) {
    srand(time(0));
    {
        ccfree::ds::Bitmap::ptr b(new ccfree::ds::Bitmap(504));
    }
...
std::vector<ccfree::ds::Bitmap::ptr> vs;
std::vector<ccfree::ds::Bitmap::ptr> vs2;
std::vector<ccfree::ds::RoaringBitmap::ptr> vs3;
...
void init() {
    ...
    vs3.resize(N);
    for(int i = 0; i < N; ++i) {
        ccfree::ds::Bitmap::ptr r(new ccfree::ds::Bitmap(M));
        ccfree::ds::RoaringBitmap::ptr r2(new ccfree::ds::RoaringBitmap());
        for(int i = 0; i < M / 1; ++i) {
            uint32_t v = rand() % M;
            r->set(v, true);
            r2->set(v, true);
...
void write_to_file(const std::string& name) {
    {
        ...
        a.reset(new ccfree::ds::Bitmap(0));
        a->readFrom(ba);
...

    {
        ...
        a.reset(new ccfree::ds::RoaringBitmap);
        a->readFrom(ba);
...
void load_from_file(const std::string& name) {
    ...
    ba->setPosition(0);
    ccfree::ds::Bitmap::ptr a(new ccfree::ds::Bitmap(0));
    a->readFrom(ba);
    std::cout << "compress= " << a->isCompress()
              << " rate=" << a->getCompressRate()
              << " size=" << a->getSize()
...
void test_uncompress2() {
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            ccfree::ds::Bitmap::ptr b(new ccfree::ds::Bitmap(M));
            *b |= *vs2[i];
        }
    }
}
    ...
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            ...
            *b |= *vs2[i];
            ...
            CCFREE_ASSERT(*b == *vs2[i]->uncompress());
        }
    }
}
...
void test_uncompress4() {
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            ccfree::ds::Bitmap::ptr b(new ccfree::ds::Bitmap(M, 0xff));
            *b &= *vs2[i];
...
void test_roaring_bitmap() {
    ccfree::ds::RoaringBitmap::ptr rb(new ccfree::ds::RoaringBitmap);
    for(int i = 0; i < 10; ++i) {
        rb->set(rand(), true);
    }
